...
public:
	 ...
	 */
	SOESocketHandler(NetSocket::Socket* socket, std::function<void(SOESocketHandler*)> onDeath);
...
private:
	...
	std::mutex m_socketTX;
	std::unique_ptr<NetSocket::Socket> socket;
	std::function<void(SOESocketHandler*)> onDeath;

	std::thread thread_rx;
	std::thread thread_tx;

	std::mutex m_remoteReturn;
	std::condition_variable cv_remoteReturn;
	bool remoteReturn;

	std::mutex m_localPort;
	std::unique_ptr<SerialAccess::SerialPort> localPort;
